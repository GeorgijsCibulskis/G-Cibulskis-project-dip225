...
# Nedaudz cita centrēšanas funkcija, kurā var uzdota KURA rinda ir jācentrē UN LĪDZ KURAI kolonnai
def center_whole_row(sheet, workbook, filename, row, start_col, end_col):
    ...
    # Šī funkcija ņem visas šūnas (no min_col līdz max_col) no 1. rindas, tad visas no otra utt. līdz pē'dejais rindas (max_row)
...
def expanding_week_excel(workbook, sheet, filename):
    # Uztaisam tabulas katrai šūnai robežu ('border')
    border_style = Border(left=Side(style='thin'), right=Side(style='thin'), top=Side(style='thin'), bottom=Side(style='thin'))
    for row in sheet.iter_rows(min_row = sheet['F16'].value, max_row = sheet['F16'].value + 5, min_col = 1, max_col = 8):
        for cell in row:
...
def appending_nutrition(workbook, sheet, filename, driver, product, mass, end_of_day):
    ...
    # Izrādījas, ka var gadīties tā, ka, vēlreiz ievadot produkta nosaukumu (jau no piedāvāta sarakata teskta kastē) ievades laukā tīmekļa vietnē, tā vietne var atgriezt nevis uzreiz lapa ar barības vielām
    ...
    # Ar taukiem un ogļhidrātiem jau ir sarežģītāk - vajadzēja atrast tabulas rindu, tad tajā <b> elementu, un tajā tekstu, bet vissarežģītakais bija atrast kā attēlot kodā elementu &nbsp;, lai selenium
    ...
    # Pārbaudam, vai diena nav beidzies, aj ir, tad jāsamaina 'control day' un 'control column' vērtības
    if end_of_day == "Yes":
