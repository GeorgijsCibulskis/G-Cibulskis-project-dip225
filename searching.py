...
def search_for_product(driver, textbox, product_name, progressbar):
        ...
        # Pārbaudām vai produkts eksistē vietnē
        if validate_search(driver):
            progressbar.pack_forget()
            messagebox.showerror("Search Error", "Invalid input. No such product has been found")
        else:

            # Atrodam pogu '40', kura uzreiz radīs 40 populārākus un piemērotākus produktus pēc meklēšanas
            ...
            # Tātad programma atrod visas tabulas rindas un nokopē no tiem tekstu, kas principā ir garš saraksts ar ēdienu nosaukumiem, kuri tika atrasti pēc lietotāja ievadīta vārda
            ...
            # Kad produkti tika atrasti, tad mēs papildinām teksta kasti ar visiem iespējamiem izvēles variantiem
            for food in text_from_cells:
                textbox.insert('end', f"- {food}\n")
            progressbar.pack_forget()

# Šī funkcija pārbauda vai tīmekļa lapā ir paragrāfs, ar tekstu 'No matching foods found.'. Ja tāds ir, tad tas produkts, kuru ievadīja lietotājs ir nepareizs vai neeksistē vietnē
